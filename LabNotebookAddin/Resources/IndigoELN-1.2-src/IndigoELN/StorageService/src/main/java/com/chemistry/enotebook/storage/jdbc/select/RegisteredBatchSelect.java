...
public class RegisteredBatchSelect extends AbstractSelect {
	
	private static final Log log = LogFactory.getLog(RegisteredBatchSelect.class);
	
	public RegisteredBatchSelect(DataSource dataSource, String sqlQuery) {
	...
	public Object mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		regBatchInfo.setMeltPointRange(this.getTemperatureRange(rs));
		...
		regBatchInfo.setIntermediateOrTest(rs.getString("INTERMEDIATE_OR_TEST"));
	...
	private BatchResidualSolventModel getBatchResidualSolventModel(String xmlString) {
		...
		model.setAdditionOrder(Integer.parseInt(getXMLElementValueFromString(xmlString, "Addition_Order")));
		model.setCodeAndName(getXMLElementValueFromString(xmlString, "Code_And_Name"));
		
		if (xmlString.indexOf("<Comments>") > 0) {
			model.setComments(getXMLElementValueFromString(xmlString, "Comments"));
		}
		
		model.setEqOfSolvent(Double.parseDouble(getXMLElementValueFromString(xmlString, "Eq_Of_Solvent")));
	...
	private BatchSolubilitySolventModel getBatchSolubilitySolventModel(String xmlString) {
		...
		model.setQualitative(Boolean.valueOf(getXMLElementValueFromString(xmlString, "Qualitative")).booleanValue());
		model.setQuantitative(Boolean.valueOf(getXMLElementValueFromString(xmlString, "Quantitative")).booleanValue());
		
		if (model.getQualitative()) {
			model.setQualiString(getXMLElementValueFromString(xmlString, "Quali_String"));
		} else if (model.getQuantitative()) {
			String oper = getXMLElementValueFromString(xmlString, "Operator");
			model.setOperator(oper);
			model.setSolubilityUnit(getXMLElementValueFromString(xmlString, "Solubility_Unit"));
			// upper value needs to be set
			model.setSolubilityValue(Double.parseDouble(getXMLElementValueFromString(xmlString, "Solubility_Value")));
		}
	 ...
	 */
	private TemperatureRangeModel getTemperatureRange(ResultSet rs) throws SQLException {
		TemperatureRangeModel range = new TemperatureRangeModel();
		range.setUpper(CommonUtils.toDouble(rs.getString("MELT_POINT_VAL_UPPER")));
		range.setLower(CommonUtils.toDouble(rs.getString("MELT_POINT_VAL_LOWER")));
		range.setComment(rs.getString("MELT_POINT_COMMENTS"));
	...
	private BatchSubmissionContainerInfoModel getContainerSubmissionModel(String xmlString) {
		...
		if (xmlString.indexOf("<Site_Code>") > 0) {
			model.setSiteCode(getXMLElementValueFromString(xmlString, "Site_Code"));
		}
