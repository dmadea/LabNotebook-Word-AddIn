...
public class MonomerPlateRow {

	private String rowId = "";
	private String plateKey = "";
	private List<MonomerPlateWell> wells = new ArrayList<MonomerPlateWell>();
	
	public void dispose() {
		if (wells != null) {
			for (MonomerPlateWell w : wells) {
				w.dispose();
				w = null;
			}
			wells.clear();
	...
	public void addWell(MonomerPlateWell well) {
		wells.add(well);
	}
...
//				MonomerPlateWell well = (MonomerPlateWell) it.next();
//				//buff.append("<well>");
...
//			e.printStackTrace();
//		}
		// skip processing wells
		List<String> listMethodNamesToSkip = new ArrayList<String>();
		listMethodNamesToSkip.add("wells");
		TextUtils.fillBufferWithClassMethods(buff, this, false, listMethodNamesToSkip);
		// now get the well xml
		buff.append(getWellsXml());
		buff.append("</plateRow>");
		return buff.toString();
	}
	
	private String getWellsXml() {
		StringBuffer buff = new StringBuffer();
		buff.append("<wells>");
		for (Iterator<MonomerPlateWell> it = wells.iterator(); it.hasNext();) {
			MonomerPlateWell well = (MonomerPlateWell) it.next();
			buff.append(well.toXml());
		}
		buff.append("</wells>");			
