...
public class Amount2 extends CeNAbstractModel implements DeepClone, DeepCopy {
	...
	private static Log log = LogFactory.getLog(Amount2.class);
	...
	public double getConversionFactor(Unit2 toUnit) {
	 ...
	 *            String representation of a number or "" or null;
	 *            
	 * @throws NumericFormatException if the val does not translate to a proper Double value.           
	 */
		...
		if (val != null && val.length() > 0) {
			...
			if (!(tstVal.isInfinite() && tstVal.isNaN())) {
				...
				if (sigs.getNumberSignificantFigures() > 0)
					result = SignificantFigures.format(sigs.doubleValue() / unit.getStdConversionFactor(), sigs
	...
	public boolean equals(Object obj) {
		...
		if (obj instanceof Amount2) {
			...
			if (getValue().equals(tmpAmt.getValue()) && isCalculated() == tmpAmt.isCalculated() && sigDigits == tmpAmt.sigDigits
					&& getUnit().equals(tmpAmt.getUnit()))
				result = true;
		}
		return result;
	}
	...
	public int hashCode() {
		...
		int iTmp = 0;
		if (result.length > 0)
			iTmp = Integer.parseInt(result[0]);
	...
	public String toXML() {
		...
		xmlbuff.append("<Default_Value>"+ this.defaultValue +"</Default_Value>");
		//xmlbuff.append(unit.toXML());
		xmlbuff.append("<Value>" + this.value +"</Value>");
		return xmlbuff.toString();
	}

	public int getDisplayedFigs() {
		return displayedFigs;
	}

	public void setDisplayedFigs(int displayedFigs) {
		this.displayedFigs = displayedFigs;
	}

	public void setCanBeDisplayed(boolean isCanBeDisplayed) {
