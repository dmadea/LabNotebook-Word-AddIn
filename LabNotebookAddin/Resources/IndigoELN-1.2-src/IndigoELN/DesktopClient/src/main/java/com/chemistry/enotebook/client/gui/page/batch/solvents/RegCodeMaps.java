...
public class RegCodeMaps {
	...
	private HashMap handlingMap = null;
	private HashMap hazardMap = null;
	private HashMap residualSolventMap = null;
	private HashMap stereoisomerCodeMap = null;
	private HashMap saltCodeMap = null;
	 ...
	 * 
	 */
	public void buildSICMap() {
		try {
			stereoisomerCodeMap = new FastHashMap();
			CachedRowSet sicRowSet = codeTableCache.getCacheAsRowSet(CodeTableCache.STEREOISOMERS);
			if (sicRowSet != null) {
				synchronized (sicRowSet) {
					sicRowSet.beforeFirst();
					while (sicRowSet.next()) {
						stereoisomerCodeMap.put(sicRowSet.getString("STEREOISOMER_CODE"), sicRowSet.getString("STEREOISOMER_DESC"));
					}
				}
			}
		} catch (Exception e) {
	...
	public void buildCompoundSourceList() {
		try {
			...
			for (Iterator<Properties> it = list.iterator(); it.hasNext();) {
				Properties rec = (Properties) it.next();
				compoundSourceList.add(new Object[] { 
						rec.get(CodeTableCache.SOURCE__SOURCE_CODE), 
						rec.get(CodeTableCache.SOURCE__SOURCE_DESC)  });
			}
		} catch (Exception e) {
			CeNErrorHandler.getInstance().logExceptionMsg(null, e);
		}
	 ...
	 */
	public HashMap getStereoisomerCodeMap() {
		if (stereoisomerCodeMap == null)
			buildSICMap();
