 ...
 *
 */
public class PCeNAmountComponent extends JPanel implements PopupMenuListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = 8913746691893898600L;
	private static Logger logger = Logger.getLogger(PCeNAmountComponent.class.getName());
	...
	public PCeNAmountComponent() {
		super();
		init();
	}

	public PCeNAmountComponent(AmountModel amt) {
		super();
		init();
		setValue(amt);
	}
	...
	public void setValue(Object value) {
		...
		if (value instanceof AmountModel) {
			if (debug)
				System.out.println("AmountComponent.setValue(Object) setting Amount of : " + ((AmountModel) value).getValue()
						+ " unit = " + ((AmountModel) value).getUnit());
			// vbtodo  won't this throw and null pointer exception is currentAmt == null?
...
//		setSelectionColor(c);
	...
	public void setFontColor(Color clr) {
		// vbtodo is this overriding the blue?
	...
	public void updateDisplay() {
		...
		if (currentAmt != null) {
			...
			// valueView.setText(Double.toString(currentAmt.getValue()));
			...
			if ( ! ( currentAmt.isCalculated()  
				...
				valueView.setText(currentAmt.getValue()); // currentAmt.toString()
			} else
				valueView.setText("");
		...
		// we should have already done this setColors();  // vb 11/14
		refresh();
	}
	...
	protected boolean compareValue(Object value) {
		...
		if (currentAmt != null) {
			if (value instanceof AmountModel) {
				if (debug)
					System.out.println("AmountComponent.compareValue(Object) comparing Amount of : " + ((AmountModel) value).getValue()
	...
	protected void debugState(String source) {
		...
		if (debug) {
			...
			System.out
					.println("AmountComponent.debugState(): currentAmt.getValue() = " + currentAmt.getValue());
